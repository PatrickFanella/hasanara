...
def test_cookie_unsafe_routes_require_valid_csrf_before_downstream_auth(client, method, path, kwargs):
    ...
    assert (
        request(
            path, cookies=cookies, headers={"Origin": settings.FRONTEND_ORIGIN, "X-CSRF-Token": "bad"}, **kwargs
        ).json()["error"]
        == "csrf_failed"
    )
...
def test_api_key_exemption_requires_a_valid_key_and_no_session_cookie(client, db_session):
    ...
    db_session.execute(
        text(
            "INSERT INTO users (id, email, oauth_provider, oauth_subject) VALUES (:id, 'csrf-key@example.com', 'google', 'csrf-key')"
        ),
        {"id": user_id},
    )
    db_session.execute(
        text(
            "INSERT INTO api_keys (id, user_id, name, key_hash, key_prefix) VALUES (:id, :user_id, 'CSRF', :hash, :prefix)"
        ),
    ...
    authorization_mixed = client.post(
        "/jobs", headers={"Authorization": "Bearer auth-looking"}, cookies={"tc_session": "session"}, json={}
    )
...
def test_cookie_authenticated_logout_requires_and_accepts_csrf(client, db_session):
    ...
    db_session.execute(
        text(
            "INSERT INTO users (id, email, oauth_provider, oauth_subject) VALUES (:id, 'csrf-logout@example.com', 'google', 'csrf-logout')"
        ),
        {"id": user_id},
    )
    db_session.execute(
    ...
    assert (
        db_session.execute(text("SELECT COUNT(*) FROM sessions WHERE user_id=:user_id"), {"user_id": user_id}).scalar()
        == 0
    )
