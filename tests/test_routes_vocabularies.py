    ...
    def execute(self, statement, params=None):
        ...
        if "INSERT INTO user_vocabularies" in sql:
            ...
            self.rows[str(params["id"])] = {
                ...
                "user_id": uuid.UUID(str(params["user_id"])) if params["user_id"] else None,
                "name": params["name"],
                "terms": [],
        ...
        if "WHERE id = :id" in sql or "WHERE id=:id" in sql:
...
def test_only_admin_can_create_or_delete_global_vocabulary(vocabulary_client):
    ...
    current_user["value"] = users["owner"]
    assert client.delete(f"/vocabularies/{created.json()['id']}").status_code == 403
