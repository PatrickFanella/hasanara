...
from app.archive.labeling.extractors import (
    ...
    windows = [
        {
            "id": "w1",
            "video_id": "v1",
            "text": "okbuddy starts the segment and okbuddy is the joke",
            "start_ms": 0,
            "end_ms": 60000,
        },
        {
            "id": "w2",
            "video_id": "v2",
            "text": "people keep saying okbuddy during this stream",
            "start_ms": 0,
            "end_ms": 60000,
        },
        {
            "id": "w3",
            "video_id": "v3",
            "text": "chadvice call segment and chadvice question",
            "start_ms": 0,
            "end_ms": 60000,
        },
    ]
    ...
    windows = [
        {
            "id": "w1",
            "video_id": "v1",
            "text": "what not just know going this is in the",
            "start_ms": 0,
            "end_ms": 60000,
        },
        {
...
def test_alias_extractor_rejects_low_information_published_aliases():
    ...
    aliases = [
        ...
        {
            "label_id": "junk-2",
            "label": "S Actually Really",
            "alias": "s actually really",
            "kind": "topic",
            "status": "active",
        },
...
def test_normalization_rejects_junk_phrases_and_slugifies_labels():
    ...
    assert is_junk_phrase("know")
    assert is_junk_phrase("okay")
    assert is_junk_phrase("that's")
    assert is_junk_phrase("number")
    assert is_junk_phrase("all of these")
    assert is_junk_phrase("something")
...
def test_title_alias_extractor_emits_title_source_candidates():
    ...
    candidates = extract_title_alias_candidates(
        {"id": "video-1", "title": "HasanAbi April 23, 2026 – hanging out after Chadvice"}, aliases
    )
...
def test_title_person_suggester_finds_names_and_filters_hasan_noise():
    names = suggest_person_names_from_title(
        "HasanAbi May 27, 2026 – back in LA, Senate candidate Julie Gonzales visits"
    )
