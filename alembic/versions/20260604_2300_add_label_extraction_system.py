...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint(
            "scope IN ('video', 'batch', 'period', 'backfill')", name="archive_extraction_runs_scope_check"
        ),
        sa.CheckConstraint(
            "extraction_tier IN ('cheap', 'balanced', 'premium')", name="archive_extraction_runs_extraction_tier_check"
        ),
        sa.CheckConstraint(
            "status IN ('running', 'completed', 'failed', 'cancelled')", name="archive_extraction_runs_status_check"
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.ForeignKey("archive_labels.id", ondelete="SET NULL"),
            nullable=True,
        ),
        sa.Column(
            "canonical_id",
        ...
        sa.Column(
            "created_by_run_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("archive_extraction_runs.id", ondelete="SET NULL"),
            nullable=True,
        ),
        ...
        sa.CheckConstraint(
            "publish_tier IN ('gold', 'silver', 'bronze', 'shadow')", name="archive_labels_publish_tier_check"
        ),
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            ...
            "end_ms",
            "text_hash",
            name="archive_transcript_windows_video_source_start_end_hash_uq",
        ),
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "status IN ('candidate', 'published', 'rejected', 'hidden')", name="archive_video_chapters_status_check"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "label_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("archive_labels.id", ondelete="CASCADE"),
            nullable=False,
        ),
        ...
        sa.CheckConstraint(
            "source IN ('admin', 'automatic', 'hybrid', 'seed')", name="archive_label_aliases_source_check"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "chapter_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("archive_video_chapters.id", ondelete="CASCADE"),
            nullable=True,
        ),
        sa.Column(
            "window_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("archive_transcript_windows.id", ondelete="CASCADE"),
            nullable=True,
        ),
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.CheckConstraint(
            "unit_type IN ('vod', 'chapter', 'window', 'segment')", name="archive_label_assignments_unit_type_check"
        ),
        sa.CheckConstraint(
        ...
        sa.CheckConstraint(
            "publish_tier IN ('gold', 'silver', 'bronze', 'shadow')",
            name="archive_label_assignments_publish_tier_check",
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "assignment_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("archive_label_assignments.id", ondelete="SET NULL"),
            nullable=True,
        ),
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            ...
            "unit_type",
            "extraction_tier",
            name="archive_label_policies_label_kind_unit_type_extraction_tier_uq",
        ),
        sa.CheckConstraint(
            "label_kind IN ('topic', 'person', 'series', 'category', 'event', 'game', 'org', 'meme', 'place', 'issue')",
            name="archive_label_policies_label_kind_check",
        ),
        sa.CheckConstraint(
            "unit_type IN ('vod', 'chapter', 'window', 'segment')", name="archive_label_policies_unit_type_check"
        ),
        sa.CheckConstraint(
            "extraction_tier IN ('cheap', 'balanced', 'premium')", name="archive_label_policies_extraction_tier_check"
        ),
    )
