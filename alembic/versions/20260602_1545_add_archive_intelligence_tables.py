...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.ForeignKey("archive_topics.id", ondelete="CASCADE"),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.UniqueConstraint(
            "topic_id", "video_id", "segment_id", "start_ms", name="archive_topic_mentions_topic_video_segment_start_uq"
        ),
    )
    op.create_table(
        ...
        sa.UniqueConstraint(
            "topic_id", "period", "granularity", name="archive_topic_period_stats_topic_period_granularity_uq"
        ),
    )
    op.create_table(
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS archive_topic_aliases_alias_lower_idx ON archive_topic_aliases (LOWER(alias))"
    )
    op.execute(
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS archive_topic_mentions_occurred_at_idx ON archive_topic_mentions (occurred_at DESC)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS archive_topic_period_stats_period_trend_idx ON archive_topic_period_stats (period, trend_score DESC)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS archive_search_trends_period_trend_idx ON archive_search_trends (period, trend_score DESC)"
    )
    op.execute(
